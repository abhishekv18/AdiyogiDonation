...
function SkeletonMarquee() {
  return (
    <div className="sp-marquee-wrap">
      <div className="skeleton-marquee-track">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="skeleton-logo-card">
...
function SkeletonSponsorGrid() {
  return (
    <div className="sp-grid">
      {[...Array(6)].map((_, i) => (
        <div key={i} className="skeleton-sp-card">
          <div className="skeleton-shimmer-inner" />
        </div>
      ))}
    </div>
...
/* ── Marquee strip for sponsors ─────────────────────────── */
function SponsorMarquee({ sponsors }) {
  if (!sponsors.length) return <SkeletonMarquee />;
  ...
  return (
    <div className="sp-marquee-wrap">
      ...
      >
        {doubled.map((s, i) => (
          <div key={i} className="sp-logo-card">
            <img src={s.img} alt={s.name} className="sp-logo-img" />
          </div>
        ))}
      </motion.div>
        ...
        if (Array.isArray(images) && images.length > 0) {
          const gallerySlides = images
            ...
            .map((image, index) => {
              ...
              return {
                _id: image._id || image.id || `sponsor-${index}`,
                name: image.title || image.label || image.alt || `Partner ${index + 1}`,
                img: imageUrl,
                alt: image.alt || image.title || `Partner ${index + 1}`,
                category: image.category || "Supporting Partner",
                website: image.website || null,
              };
            });
          
          cacheRef.current = gallerySlides;
          setSponsors(gallerySlides);
        } else {
          cacheRef.current = [];
          setSponsors([]);
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .skeleton-marquee-track {
          display: flex; align-items: center; gap: 12px;
          white-space: nowrap; padding: 4px 0;
        }
        @media (min-width: 640px) {
          .skeleton-marquee-track { gap: 16px; padding: 6px 0; }
        }
        
        .skeleton-logo-card {
          display: inline-flex; align-items: center; justify-content: center;
          min-width: 150px; height: 76px; border-radius: 12px;
          background: #e2e8f0; border: 1px solid var(--border);
          flex-shrink: 0; position: relative; overflow: hidden;
        }
        @media (min-width: 640px) {
          .skeleton-logo-card { min-width: 170px; height: 80px; border-radius: 14px; }
        }
        @media (min-width: 1024px) {
          .skeleton-logo-card { min-width: 180px; height: 84px; border-radius: 16px; }
        }
        ...
        @media (min-width: 640px) {
          .skeleton-sp-card { padding: 20px 14px; border-radius: 16px; min-height: 115px; }
        }
        @media (min-width: 1024px) {
          .skeleton-sp-card { padding: 22px 16px; border-radius: 18px; min-height: 120px; }
        }
        ...
        /* All the donation card and modal styles remain the same */
        .donate-card {
          position: relative;
          border-radius: 20px;
          background: #fff;
      ...
      <section id="donate" className="sd-section">
        ...
        <div className="sponsors-block">
          <div className="sponsors-inner">
            ...
            {isSponsorsLoading ? <SkeletonMarquee /> : <SponsorMarquee sponsors={sponsors} />}
          </div>
        </div>
        ...
        <div className="sd-lower">
          ...
          >
            ...
            {/* Sponsor Grid - Shows only 6 sponsors or skeleton */}
            {isSponsorsLoading ? (
              <SkeletonSponsorGrid />
