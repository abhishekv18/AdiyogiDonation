...
const guestChartRef = useRef(null);
const donationChartRef = useRef(null);
const donationPieChartRef = useRef(null);
const contactsChartRef = useRef(null);
const recentDonationsChartRef = useRef(null);

// For downloading all charts
const allChartRefs = [guestChartRef, donationChartRef, donationPieChartRef, contactsChartRef, recentDonationsChartRef];
...
// Add this helper function before your chart download functions
const convertOklchToRgb = (element) => {
  // Replace Tailwind's oklch colors with standard colors
  const styles = window.getComputedStyle(element);
  const colorProps = ['color', 'backgroundColor', 'borderColor', 'boxShadow'];
  
  // Map of common Tailwind oklch colors to hex
  const colorMap = {
    // Teal colors
    'oklch(0.704 0.14 182.503)': '#14B8A6',
    'oklch(0.6 0.118 184.704)': '#0D9488',
    'oklch(0.511 0.096 186.391)': '#0F766E',
    // Slate colors
    'oklch(0.446 0.043 257.281)': '#64748B',
    'oklch(0.372 0.044 257.287)': '#475569',
    'oklch(0.279 0.041 260.031)': '#334155',
    'oklch(0.208 0.042 265.755)': '#1E293B',
    // Emerald
    'oklch(0.596 0.145 163.225)': '#059669',
    // Cyan
    'oklch(0.715 0.143 215.221)': '#06B6D4',
    // Rose
    'oklch(0.645 0.246 16.439)': '#E11D48',
    // Amber
    'oklch(0.769 0.188 70.08)': '#D97706',
    // White/Black
    'oklch(1 0 0)': '#FFFFFF',
    'oklch(0 0 0)': '#000000',
  };
  ...
  const processElement = (el) => {
    ...
    colorProps.forEach(prop => {
      ...
      if (value && value.includes('oklch')) {
        // Replace oklch colors in the value
        let newValue = value;
        Object.entries(colorMap).forEach(([oklch, hex]) => {
          newValue = newValue.replace(new RegExp(oklch.replace(/[()]/g, '\\$&'), 'g'), hex);
        });
        // Also handle any other oklch values by converting to a fallback
        newValue = newValue.replace(/oklch\([^)]+\)/g, '#CBD5E1'); // Fallback to slate-300
        updates.push({ prop, value: newValue });
        needsUpdate = true;
      }
    });
...
// Chart Download Utility - Updated with better error handling
const downloadChartAsImage = async (chartRef, filename = 'chart.png') => {
  ...
  try {
    const canvas = await html2canvas(chartRef.current, {
      ...
      allowTaint: true,
      useCORS: true,
      foreignObjectRendering: false, // Add this
      onclone: (clonedDoc) => {
        // Fix SVG elements in the clone
...
// Alternative: Download chart data as SVG directly
const downloadChartAsSVG = (chartRef, filename = 'chart.svg') => {
  if (!chartRef.current) return;
...
// Download all charts as a combined image
const downloadAllCharts = async (chartRefs, filename = 'dashboard-charts.png') => {
  try {
    toast.loading('Preparing charts for download...');
    ...
    `;
    title.innerHTML = `
      <h1 style="margin: 0 0 8px 0; font-size: 28px; font-weight: bold;">Adiyogi Foundation</h1>
      <p style="margin: 0; font-size: 16px; opacity: 0.9;">Dashboard Charts Report</p>
    `;
    ...
    // Add each chart image to container
    ...
    footer.innerHTML = `
      <p style="margin: 0;">
        Generated on ${new Date().toLocaleString('en-IN', { 
          dateStyle: 'full', 
          timeStyle: 'short' 
        })} | Adiyogi Foundation Admin Panel
      </p>
    `;
    ...
    toast.dismiss();
    toast.success('All charts downloaded successfully');
  } catch (error) {
    console.error('Failed to download all charts:', error);
    toast.dismiss();
    
    // Fallback: Try downloading charts individually
    toast.error('Combined download failed. Try downloading charts individually.');
  }
};
...
const handleLogout = async () => {
  ...
  //   </section>
  // );
// const ChartCard = ({ title, children, chartRef, filename, className = "" }) => (
//   <section className={`rounded-xl border border-slate-200 bg-white p-3 sm:p-5 shadow-sm ${className}`}>
//     <div className="flex items-center justify-between mb-3 sm:mb-4">
//       <h3 className="text-sm sm:text-base font-semibold text-slate-900">{title}</h3>
//       <button
//         onClick={() => downloadChartAsImage(chartRef, filename)}
//         className="flex items-center gap-1.5 rounded-lg border border-slate-200 px-2.5 py-1.5 text-[11px] sm:text-[12px] font-medium text-slate-600 transition-all hover:bg-teal-50 hover:text-teal-600 hover:border-teal-200"
...
const ChartCard = ({ title, children, chartRef, filename, className = "" }) => {
    ...
    <section className={`rounded-xl border border-slate-200 bg-white p-3 sm:p-5 shadow-sm ${className}`}>
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => downloadChartAsSVG(chartRef, filename.replace('.png', '.svg'))}
            className="flex items-center gap-1.5 rounded-lg border border-slate-200 px-2.5 py-1.5 text-[11px] sm:text-[12px] font-medium text-slate-600 transition-all hover:bg-teal-50 hover:text-teal-600 hover:border-teal-200"
  ...
  {allChartRefs.some(ref => ref.current) && (
      <div className="flex justify-end">
        <button
          onClick={() => downloadAllCharts(allChartRefs, 'adiyogi-dashboard-charts.png')}
          className="inline-flex items-center gap-2 rounded-lg bg-teal-600 px-4 py-2 text-[13px] font-semibold text-white shadow-sm transition-all hover:bg-teal-700"
        >
          <Download size={16} />
          Download All Charts
        </button>
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
        ...
        <StatsCard 
          ...
          icon={Users}
          subtitle="Beneficiaries of the Food Distribution Program"
        />
        <StatsCard 
      ...
      chartRef={guestChartRef}
      filename="guest-registration-trend.png"
    >
      <ResponsiveContainer width="100%" height={250} className="sm:h-[300px] lg:h-[350px]">
        <LineChart data={groupByMonth(allGuests, "guests")}>
          <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
    ...
    <div className="grid gap-4 sm:gap-5 grid-cols-1 lg:grid-cols-2">
      {/* Donation Trends - with ref */}
      <ChartCard 
        title="Donation Trends" 
        chartRef={donationChartRef}
        filename="donation-trends.png"
      >
        <ResponsiveContainer width="100%" height={250} className="sm:h-[300px]">
          <BarChart data={donationAmountData}>
            <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
      ...
      {/* Donation Distribution - with ref */}
      <ChartCard 
        title="Donation Distribution" 
        chartRef={donationPieChartRef}
        filename="donation-distribution.png"
      >
        {getDonationTierData().length > 0 ? (
          <ResponsiveContainer width="100%" height={250} className="sm:h-[300px]">
            <PieChart>
    ...
    {/* Contact Messages Trend - with ref */}
    <ChartCard 
      title="Contact Messages Trend" 
      chartRef={contactsChartRef}
      filename="contact-messages-trend.png"
    >
      <ResponsiveContainer width="100%" height={250} className="sm:h-[300px]">
        <BarChart data={contactsData}>
          <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
    ...
    {/* Recent Donations - with ref */}
    {allDonors.length > 0 && (
      <ChartCard 
        title="Recent Donations" 
        chartRef={recentDonationsChartRef}
        filename="recent-donations.png"
      >
        <div className="overflow-x-auto -mx-4 sm:mx-0">
          <div className="inline-block min-w-full align-middle">
            <table className="min-w-full text-left">
