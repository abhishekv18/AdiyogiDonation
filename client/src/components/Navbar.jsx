...
//         // Filter only donations within last 48 hours
//         const validDonors = Array.isArray(donors) 
//           ? donors.filter(donor => donor.createdAt && isWithinLast48Hours(donor.createdAt))
//           : [];
        
//         // Sort by most recent first
//         const sorted = validDonors.sort((a, b) => 
...
//   // If no recent donations, don't show ticker
//   if (!isLoading && recentDonations.length === 0) {
//     return null;
//   }

//   // Repeat array 3 times for smooth infinite scroll
//   const repeatedDonations = recentDonations.length > 0 
...
//   //         <span
//   //           className="flex items-center gap-2 rounded-full px-4 py-1 text-[11px] font-bold uppercase tracking-widest text-white"
//   //           style={{
//   //             background: "linear-gradient(135deg, #D81B4A 0%, #C01640 100%)",
//   //             boxShadow: "0 2px 12px rgba(216,27,74,0.30)",
//   //           }}
//   //         >
//   //           <FiHeart size={10} />
...
//   //     {/* Left fade */}
//   //     <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-32 bg-gradient-to-r from-[#FAFAF7] to-transparent" />

//   //     {/* Right fade */}
//   //     <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-32 bg-gradient-to-l from-[#F3F5EF] to-transparent" />
...
//   //         animate={{ x: ["0%", "-33.33%"] }}
//   //         transition={{
//   //           duration: window.innerWidth < 640 ? 12 : 22,
//   //           ease: "linear",
//   //           repeat: Infinity,
//   //         }}
//   //       >
...
//   //             <span
//   //               key={`${donor._id || i}-${i}`}
//   //               className="inline-flex items-center gap-2.5 px-7 text-[13px] font-medium group cursor-default"
//   //               style={{ color: "#64748B" }}
//   //               title={`Donated ${timeAgo}`}
//   //             >
//   //               {/* Animated dot */}
...
//   //                   style={{
//   //                     background: "linear-gradient(135deg, #8AAE3B, #D81B4A)",
//   //                   }}
//   //                 />
//   //               </span>

//   //               <span 
//   //                 className="font-semibold transition-colors group-hover:text-[#D81B4A]" 
//   //                 style={{ color: "#1E293B" }}
//   //               >
//   //                 {donorName}
//   //               </span>
...
//   //               {timeAgo && (
//   //                 <span 
//   //                   className="text-[10px] opacity-0 group-hover:opacity-100 transition-opacity px-2 py-0.5 rounded-full"
//   //                   style={{ 
//   //                     color: "#94A3B8",
...
//         <div className="absolute left-1/2 top-1/2 z-20 -translate-x-1/2 -translate-y-1/2">
//           <span
//             className="flex items-center gap-1.5 rounded-full px-3.5 py-1 text-[10px] font-bold uppercase tracking-widest text-white"
//             style={{
//               background: "linear-gradient(135deg, #D81B4A 0%, #C01640 100%)",
//               boxShadow: "0 2px 12px rgba(216,27,74,0.30)",
//             }}
...
//       {/* Right fade */}
//       <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-24 bg-gradient-to-l from-[#F3F5EF] to-transparent" />
...
//           animate={{ x: ["0%", "-33.33%"] }}
//           transition={{
//             duration: window.innerWidth < 640 ? 8 : 16,
//             ease: "linear",
//             repeat: Infinity,
//           }}
//         >
...
//               <span
//                 key={`${donor._id || i}-${i}`}
//                 className="inline-flex items-center gap-1.5 mx-4 text-[13px] font-medium group cursor-default shrink-0"
//                 style={{ color: "#64748B" }}
...
//                 <span className="text-[10px] opacity-50 mx-0.5" style={{ color: "#94A3B8" }}>•</span>
...
//                 {timeAgo && (
//                   <span 
//                     className="text-[10px] opacity-0 group-hover:opacity-100 transition-opacity ml-1 px-1.5 py-0.5 rounded-full"
//                     style={{ 
//                       color: "#94A3B8",
//                       background: "rgba(148,163,184,0.1)",
//                     }}
...
//         }
//         .brand-logo-wrap img { width: 32px; height: 32px; object-fit: contain; }
...
function DonationTicker() {
  ...
  const displayDonations = showRecent && recentDonations.length > 0 
    ? recentDonations 
    : oldDonations.length > 0 
    ? oldDonations 
    : [];
  ...
  const getLabelText = () => {
    if (showRecent && recentDonations.length > 0) {
      return recentDonations.length === 1 ? "Recent" : `${recentDonations.length} Recent`;
    }
    return oldDonations.length === 1 ? "Donation" : `${oldDonations.length} Donations`;
  };
